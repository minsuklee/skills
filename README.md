...
- L5 미의 상한선으로 감점 지시가 `못생기게 그려줘`로 해석되는 것을 막음 — 실사감과 매력이 상충하지 않게 하는 마지막 한 줄
- 손가락·눈동자 하이라이트·글자·피부 균질도 등 여덟 가지 생성 후 점검 항목. 걸린 항목만 긍정문으로 보강해 재생성
- 같은 프롬프트도 장마다 편차가 크므로 3~5장 병렬 생성을 기본 운용으로 잡음
- `scripts/build_prompt.py` 로 장면·상세도·셀카 여부·언어를 지정해 변형 N개 프롬프트를 한 번에 뽑음
